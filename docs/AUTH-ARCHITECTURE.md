...
| P0b | **AVAIL-07** 🟡 | Atomic single-lock `HoldCommitted` (**one event**, no release-then-reinsert gap), excluding only that hold from the conflict check. | **Half done, fact-checked 2026-08-27 against `engine/mutations.rs:284`.** The in-memory TOCTOU is closed: `commit_hold` holds one write guard across `check_no_conflict_excluding` and both appends, which share one fsync. What remains is the *single event*: it still writes `HoldReleased` + `BookingConfirmed`, so a torn write loses the booking (fails safe, frees the slot, INV-01 holds). Payment capture is specified to fire on the first-time `HoldCommitted` append, so PAY stays gated on this. |
