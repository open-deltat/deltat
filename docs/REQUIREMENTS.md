...
- **MODEL-03** ✅ `capacity: u32` = max concurrent allocations; the **kernel requires an explicit value**: "default 1" is an edge/SQL concern, not a kernel property (`model.rs:79,87`). **`capacity >= 1` is enforced** at create and update (`validate_capacity`, LimitExceeded): 0 reads as "not bookable" but every `<= 1` branch would silently treat it as a bookable single slot, so it is rejected rather than given a surprise meaning. Replay applies pre-existing WAL rows verbatim (a durable capacity-0 resource keeps the legacy behaves-as-1 semantics). Tests: `create_resource_capacity_zero_rejected`, `update_resource_capacity_zero_rejected`.
- **MODEL-04** ✅ `buffer_after: Option<Ms>` (0 when None) extends each allocation's **effective end** (`span.end + buffer`); it is not a separate interval. `buffer_after` and a hold's `expires_at` are **bounded at the write boundary** (`validate_buffer` / `validate_timestamp` in `conflict.rs`; `[0, MAX_SPAN_DURATION_MS]` / the valid window), and every `span.end + buffer` site uses **saturating** arithmetic, so an out-of-range value (incl. one replayed from an old WAL) cannot overflow `i64` and panic the task, with `overflow-checks = true` in `[profile.release]` so any slip surfaces loudly. (Closed a DoS reachable from untrusted SQL.)
...
- **AVAIL-07** ✅ **`commit_hold(hold_id, booking_id, label)`** (`engine/mutations.rs`): hold→booking under **one resource write lock**, excluding that hold from its own conflict check: this closes the **in-memory** `release_hold`→`confirm_booking` TOCTOU (no other booker can interleave). `HoldReleased` + `BookingConfirmed` share **one fsync** (`WalCommand::AppendAtomic`): an fsync error or a pre-flush crash leaves neither durable. They are **two records, not one**, so a torn write between them is *not* both-or-neither: replay discards the torn tail; because release is written before confirm, the residual rare-crash outcome is a freed (re-bookable) slot, **never a hold-plus-booking / overbook (INV-01 holds)**. True torn-write atomicity would need a single CRC-framed record or a commit marker (deferred; same best-effort durability as the existing `flush_batch` group commit). Tested incl. the self-exclusion (mutation-verified), expired-hold rejection, clean-replay durability, and the torn-write safe-direction. **Exposed over SQL** as `UPDATE holds SET booking_id = $1 [, label = $2] WHERE id = $3` (`Command::CommitHold`, tag `UPDATE 1`), on both the simple and extended query paths; authorization is possession of the `hold_id`, the same trust the existing `DELETE FROM holds` already grants (SEC-03). The tap SDK and the demo WS path (PROTO-15) still need a `holds.commit()` that issues this statement.
- **AVAIL-08** ✅ Hold expiry is **authority-clamped**: `place_hold` still takes `expires_at: Ms` as a caller arg (wire compatibility) but clamps it to `now + max_hold_ttl_ms` from the server's injected clock (`DELTAT_MAX_HOLD_TTL_MS`, default 1 hour), so a skewed or hostile client clock cannot park an unreapable hold. Replay keeps the durably written (clamped) value verbatim. The fully authority-assigned `ttl` contract (client sends a duration, server returns the expiry) remains the v2 wire format (FORMAT.md §5.1).
...
- **AVAIL-16** ✅ **Admission honors rules (T-03 resolved).** `PlaceHold`, `ConfirmBooking`, and every batch member pass `check_rules_admit` before the allocation conflict check: the candidate's **raw span** must lie inside the effective open windows (own non-blocking base when the resource defines any rule anywhere, else the nearest scheduling ancestor's, minus every blocking window own + inherited), assembled by the same helpers `availability()` uses (`own_rule_spans` / `assemble_open_windows`), so read and write cannot drift. Rejection is `ClosedBySchedule`. Two deliberate, documented asymmetries with the read view: **(a) unscheduled chains** (no non-blocking rule anywhere) admit by collision only, blocking rules still reject — the read path reports zero availability for such a resource because it has no windows to enumerate (collision-detector mode); **(b) the turnaround buffer is exempt** — only the raw span is schedule-checked, cleanup may run outside open hours, while the buffered footprint remains the allocation check's concern. `commit_hold` re-checks allocations only: the hold **is** the admission, and a blocking rule added after placement does not revoke it (exactly as it does not revoke an existing booking). Property-tested (`verify.rs::read_path_agrees_with_write_path`, now generating partial open windows + blocking rules): an admitted write reads free over its whole raw span; a probe whose whole buffered footprint reads free is admitted.
...
- **PROTO-10** 🟡 Current external transport: PostgreSQL wire (pgwire 0.37, simple + extended query); SQL parsed by `sqlparser 0.59` (PostgreSqlDialect) into the **20-variant `Command` enum**, now defined in **`src/command.rs`** (transport-neutral: depends only on `model` + `ulid`, never `sqlparser`). `sql.rs` is one producer (`parse_sql` SQL → `Command`); `wire.rs::execute_command` consumes it.
...
- **PROTO-15** 🟡 The demo hold→confirm→release lifecycle is a per-connection **WebSocket** protocol in `tap/demo/server.ts` (`{hold|subscribe|confirm}` → `{confirmed|error|Event}`); non-atomic here (the kernel's atomic `commit_hold` is now exposed over SQL as `UPDATE holds SET booking_id = ...`, AVAIL-07, but this WS path is not yet switched to it) and hold expiry is client-supplied (`Date.now()+300000`).
...
- **EDGE-05** ✅ Human-readable display (times/dates/weeks) is computed in the **viewer's local timezone** via `@open-deltat/shared` (`formatTime(ms)`, `toLocalDateString`, `dayBounds`, `weekStart/End/Days`); the kernel emits only ms.
...
- **EX-17** 🟡 **The demo layout is state-independent: interaction changes contents, never dimensions.** Layout shift exists only when a region's size or existence depends on transient state. `{selected && <Panel/>}` conditionally mounts a panel into flow and shoves everything below it. The fix is not to reserve space after the fact; it is to never make the region conditional in the first place. A region is one of two things: (a) a permanent slot in the layout skeleton whose size does not depend on its contents (sized to its known max, or a fixed box that scrolls its overflow) with content flowing into it, or (b) out of flow entirely, an overlay whose size cannot affect layout by definition (the `Stage` `tray`: `position: fixed/absolute` with `pb-28` clearance beneath it). The irreducible tradeoff: you cannot have in-flow AND content-sized AND no-shift at once when content height changes, so choose which to drop. An overlay drops in-flow; a fixed slot drops content-sized. Techniques that give a slot a state-independent size: a measured `min-height` with an empty-state placeholder; `visibility: hidden` on a same-shape element (reserves exact space with no magic number, and unlike `opacity: 0` it also leaves the a11y tree and hit-testing); or both states stacked in one CSS grid cell so it self-sizes to the taller. Highlights use `ring`/`outline` (a box-shadow, no box growth), never a toggled `border` width; values that change in place (timers, counts, "n/a" to "12 ms") use `tabular-nums` and a reserved width. Why: the demos are the adoption and few-shot surface (VIS-10); a jumpy demo undersells the product and is what a model learns to reproduce.
...
- **TEST-12** 🟡 deltat coverage is **example-based plus one property test**: ~330 inline unit tests + **17 integration tests** (`listen_notify`, full pgwire path). The availability spec is property-tested with `proptest` (a dev-dependency; TEST-01, 2000 cases/run), so the earlier "no property deps" note was wrong. **No** simulation/mutation/fuzz tooling yet. The same-resource hold→confirm race is now covered by `concurrent_bookings_on_capacity_one_admit_exactly_one`; `compact_wal_waits_for_a_locked_resource` covers compaction under a held write lock.
...
- **SEC-03** ✅ Hold-capability model, decided: possession of the `Ulid` `hold_id` authorizes `CommitHold`, exactly as it already authorizes release (`DELETE FROM holds`) and as any interval id authorizes its delete. No separate capability secret under the single-shared-password trust model; revisit if per-tenant credentials ever land.
...
- **T-03** ✅ **RESOLVED** (was open): the conflict check now rejects blocked and closed windows. Decision: **admission agrees with availability on the rule dimension**: a write whose raw span availability reports as unavailable (outside open hours or inside a blocking window, own or inherited) is rejected with `ClosedBySchedule`. Carve-outs, both deliberate and test-pinned: unscheduled chains (no non-blocking rule anywhere) stay bookable by collision only (blocking still rejects); the turnaround buffer tail may leave the open windows; `commit_hold` does not re-check rules (the hold is the admission). See AVAIL-16 for the full semantics and the property test.
...
- **T-06** `MODEL-12` (caller-supplied Ulids, enables `INV-06` idempotent commit) ⟷ `SEC-03` (a guessable/leaked `hold_id` = slot-hijack once `CommitHold` exists). The same property that buys idempotency creates a capability-security question. → resolved with SEC-03: `CommitHold` is exposed over SQL with possession-based authorization, matching release.
