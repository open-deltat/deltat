...
- **AVAIL-07** ✅ **`commit_hold(hold_id, booking_id, label)`** (`engine/mutations.rs`): atomic hold→booking under one resource write lock, excluding that hold from its own conflict check, with `HoldReleased` + `BookingConfirmed` written as **one WAL record** (`WalCommand::AppendAtomic`) so a crash leaves both or neither — closing the `release_hold`→`confirm_booking` TOCTOU. Tested incl. the exclusion (mutation-verified), expired-hold rejection, and replay durability. **Kernel-only:** not yet exposed over the transport/SDK, so the demo lifecycle is still release-then-book (PROTO-15) until wired; exposing it needs the SEC-03 hold-capability decision (T-06).
